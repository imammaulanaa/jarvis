...
export default async function Topbar() {
  ...
  return (
    <header className="h-14 flex items-center justify-between px-5 sticky top-0 z-10 border-b backdrop-blur-sm transition-colors"
      style={{
      ...
      {/* Left — search bar */}
      <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg border w-56 cursor-pointer"
        style={{ background: "var(--bg-secondary)", borderColor: "var(--border)" }}>
        <Search size={13} style={{ color: "var(--text-muted)" }} />
        <span className="text-xs font-mono-jarvis" style={{ color: "var(--text-muted)" }}>
          Search services...
        </span>
        <span className="ml-auto text-[10px] px-1.5 py-0.5 rounded font-mono-jarvis"
